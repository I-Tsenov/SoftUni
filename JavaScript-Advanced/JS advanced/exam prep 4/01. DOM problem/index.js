function solve() {
    ...
    const [lectureName, date] = document.querySelectorAll('.form-control input');
    const lectureOptions = document.querySelector("[name = 'lecture-module'");
    ...
    addBtn.addEventListener('click', (ev) => {
        ...
        if (lectureName.value !== '' && date.value !== '' && lectureOptions.value !== 'Select module') {

            const moduleName = e('h3', `${lectureOptions.value.toUpperCase()}-MODULE`);
            const lectureList = e('ul');
            const lecture = e('li', '', 'flex')
            const nameAndTime = e('h4', `${lectureName.value} - ${date.value.split('-').join('/').split('T').join(' - ')}`)
            const delBtn = e('button', 'Del', 'red')
            delBtn.addEventListener('click', (ev) => {
                const module = ev.target.parentNode.parentNode.parentNode
                const lectureList = ev.target.parentNode.parentNode
                const lecture = ev.target.parentNode

                if (lectureList.children.length === 1) {
                    delete modulesState[moduleName.textContent]
                    module.remove()
                } else {
                    lecture.remove()
                }
            ...
            lecture.appendChild(nameAndTime)
            lecture.appendChild(delBtn)

            if (!modulesState[moduleName.textContent]) {

                modulesState[moduleName.textContent] = moduleName.textContent
                const moduleDiv = e('div', '', 'module');
                lectureList.appendChild(lecture)
                moduleDiv.appendChild(moduleName)
                moduleDiv.appendChild(lectureList)
                modules.appendChild(moduleDiv)
            } else {
                const targetUl = Array.from(document.querySelectorAll('div > h3')).filter(e => e.textContent === moduleName.textContent)[0].nextSibling
                targetUl.appendChild(lecture)
                // console.log(targetUl)
                Array.from(targetUl.children).sort((a, b) => a.textContent.localeCompare(b.textContent)).forEach(e => targetUl.appendChild(e))  // not sorted by date!!!
            }
        }
    })
